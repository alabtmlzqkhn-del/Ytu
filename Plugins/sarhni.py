...
def sarhniFunc(c, m, k):
    ...
    if r.get(f'{m.chat.id}:addCustom:{m.from_user.id}{Dev_Neptune}'):
        return
    if r.get(f'{m.chat.id}:delCustom:{m.from_user.id}{Dev_Neptune}') or r.get(f'{m.chat.id}:delCustomG:{m.from_user.id}{Dev_Neptune}'):
        return
    ...
    if r.get(f'{m.chat.id}addCustomG:{m.from_user.id}{Dev_Neptune}'):
        return
    text = m.text
    name = r.get(f'{Dev_Neptune}:BotName') if r.get(f'{Dev_Neptune}:BotName') else 'fadi'
    ...
    if text == 'صارحني':
        ...
        return m.reply(f'{k} أهلين عيني「 \u206a\u206c\u206a\u206c{m.from_user.mention} 」\n{k} هذا رابط صارحني الخاص بيك', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('📩', url=f't.me/{botUsername}?start=sarhni{id}', style='primary')]]))

@Client.on_message(filters.private, group=2)
def sarhniHandlerP(c, m):
    k = r.get(f'{Dev_Neptune}:botkey')
    channel = r.get(f'{Dev_Neptune}:BotChannel') if r.get(f'{Dev_Neptune}:BotChannel') else 'fvamv'
...
def sarhniFuncP(c, m, k, channel):
    if m.text:
        ...
        if text.startswith('/start sarhni'):
            ...
            if not r.get(f'{id}:sarhni:{Dev_Neptune}'):
                ...
                a = m.reply(f'{k} دخلت هسة رابط صارحني مع 「 \u206a\u206c\u206a\u206c{get.first_name} 」\n{k} اي رسالة ترسلها لي راح احولها له بسرية تامة بدون مايعرفك\n༄', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('الغاء', callback_data='sarhni:bye', style='primary')], [InlineKeyboardButton('source fadi', url=f't.me/{channel}', style="danger")]]), quote=True)
        ...
        if r.get(f'{m.from_user.id}:sarhni') and len(text) < 1000:
            ...
            try:
                c.send_message(user_id, txt, disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('رد', callback_data=f'sarhni+rep{m.from_user.id}')], [InlineKeyboardButton('source fadi', url=f't.me/{channel}', style='danger')]]))
    ...
    if r.get(f'{m.from_user.id}:sarhni'):
        ...
        try:
            ...
            m.copy(user_id, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('رد', callback_data=f'sarhni+rep{m.from_user.id}', style='primary')], [InlineKeyboardButton('source fadi', url=f't.me/{channel}', style='primary')]]))
            return m.reply(f'{k} تمام ارسلت رسالتك بسرية تامة لـ {name}', quote=True)
        except Exception as e:
            print(e)
            return m.reply('مقدر ارسله شيء يمكن حاظرني', quote=True)
